        ...
        public FianzaValidator()
        {
            ...
            When(x => x.MontoDevuelto.HasValue, () =>
            {
                RuleFor(x => x.MontoDevuelto)
                    ...
                    // Mejora: Indicamos el monto máximo dinámicamente en el mensaje
                    .LessThanOrEqualTo(x => x.Monto)
                    .WithMessage(x => $"El monto devuelto no puede ser mayor al monto original de la fianza (${x.Monto}).");
            });
            ...
            When(x => x.FechaDevolucion.HasValue, () =>
            {
                RuleFor(x => x.FechaDevolucion)
                    ...
                    .WithMessage("La fecha de devolución no puede ser anterior a la fecha de registro.")
                    .LessThanOrEqualTo(DateTime.Today).WithMessage("La fecha de devolución no puede ser futura.");
            });

            // 7. Consistencia: Si hay monto > 0, debe haber fecha
            RuleFor(x => x.FechaDevolucion)
