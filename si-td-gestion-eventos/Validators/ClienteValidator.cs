...
namespace si_td_gestion_eventos.Validators
{
    public class ClienteValidator : AbstractValidator<ClienteVM>
    {
        ...
        public ClienteValidator(IClienteBusinessRules businessRules)
        {
            ...
            // -------------------------------------------------
            // --- REGLAS CONDICIONALES PARA PERSONA FÍSICA ---
            // -------------------------------------------------
            When(x => x.Tipo == TipoCliente.PersonaFisica, () =>
            {
                ...
                RuleFor(x => x.Apellido)
                    .NotEmpty().WithMessage("El apellido es obligatorio."); // <-- Arregla tu bug

                RuleFor(x => x.CedulaIdentidad) // <-- USA TU PROPIEDAD
            ...
            When(x => x.Tipo == TipoCliente.PersonaJuridica, () =>
            {
                ...
                RuleFor(x => x.Apellido).Empty().WithMessage("El apellido debe estar vacío para una persona jurídica.");
                RuleFor(x => x.CedulaIdentidad).Empty().WithMessage("La Cédula debe estar vacía para una persona jurídica.");
            });
        }

        // --- TUS MÉTODOS HELPER + 2 NUEVOS ---

        // (Tus métodos BeOnlyLettersAndSpaces, NotContainNumbers, BeValidCedulaFormat, BeValidPhoneNumber, HaveMinimumDigits van aquí...)
        #region Tus Métodos Helper
